...
export default function EditProfile() {
	...
	return (
		<View className='px-6 mt-6 w-full shadow-sm shadow-gray-300 '>
			...
			<Typography variant='label' className='ml-2 mb-3' color='gray'>
				...
				<View className='flex-1 items-center justify-center bg-[rgba(0,0,0,0.4)]'>
					<View className='w-80 bg-white p-6 rounded-2xl shadow-lg'>
						...
						<View className='flex-row justify-between mt-5'>
							<Button
								onPress={() => setModalVisible(!isModalVisible)}
								className='px-4 py-2 rounded-2lg'
							>
...
const ProfileListItem = ({
	...
	return (
		<TouchableOpacity
			className={`flex-row p-3.5 items-center bg-white gap-3 ${className} ${disabled && 'opacity-40'}`}
			disabled={disabled}
			onPress={onPress}
			{...rest}
		>
